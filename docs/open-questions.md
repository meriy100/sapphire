...
| I-OQ4 | Ruby へのパッケージング | DECIDED (0.1.0 provisional) | 本 OQ は **I-OQ29 / I-OQ30 / I-OQ31 / I-OQ33** のまとめとして subsume された。0.1.0 では **方式 X**（素の Rust binary を gem に同梱しない、`sapphire-runtime` gem と `sapphire` CLI の 2-artifact 配送）を採用。CLI binary は GitHub Release attach（`release-build.yml`）、`sapphire-runtime` は rubygems.org push。最終射程としての単一 gem (A) への移行は 0.2.0 以降で再訪。詳細は `docs/impl/12-packaging.md` / `docs/impl/32-release-process.md`。 |
...
| I-OQ40 | `Ruby.run` の返却形: タプル vs `Result` ADT | DECIDED (2026-04-19, I7c 着地) | `docs/impl/26-codegen-effect-monad.md` §run の Result ADT 包装。runtime 側は `[:ok, a] / [:err, e]` タプルを維持（R5 の既定）、生成コード側の `Sapphire::Prelude.run_action(action)` helper が `ADT.make(:Ok, [v])` / `ADT.make(:Err, [e])` で spec 11 §`run` の `Result RubyError a` 形に包み直す。I7c の主エントリ `Sapphire::<Main>.run_main` がこの helper 経由で結果を観測する。 |
...
| I-OQ57 | 型付き AST の持ち方 | DEFERRED-IMPL | `docs/impl/18-typecheck-hm.md` §I7 codegen との接続インターフェース。I6 は top-level binding ごとの scheme しか back-annotate しない。I7 codegen が "AST ノードごとの推論済み型" を欲しがるなら、sapphire-core 側に `typed_ast` を置いて side-table (`HashMap<Span, Ty>`) で繋ぐか、inferenced AST を別 tree として生成するかを I7 設計で確定する。 |
| I-OQ58 | `do` 展開を parse 段 / 型検査段 / codegen 段のどこでやるか | DECIDED (2026-04-19, I7a 着地) | `docs/impl/24-codegen-expr.md` §Do 記法の扱い / `docs/impl/26-codegen-effect-monad.md` §`do` 展開。I6 / I7 ともに **on-the-fly desugar** で揃える決定。AST は `Expr::Do` を保持して LSP 表示に使い、typeck と codegen がそれぞれ独立に同じ desugar 規則を適用する。I7 codegen 側の desugar は `expr::desugar_do` に一本化。コスト（二重 desugar）は実測で問題なく、AST 表現を失うデメリットの方が重いと判断。 |
| I-OQ59 | constraint propagation の粒度 | DEFERRED-IMPL | `docs/impl/20-typecheck-classes.md` §constraint resolution。現行 `simplify` は wanted を 1 回走査するだけで、instance 経路が開いた後に再度積み上がった constraint を再試行しない。M9 範囲では十分だが、superclass が深い階層を持つ入れ子では不足しうる。I7 codegen で "evidence chain を生成する" 段階で本格化する必要がある場合、`simplify` を fixed-point に拡張する。 |
...
| I-OQ80 | Operator と type class dictionary passing | DEFERRED-IMPL | `docs/impl/24-codegen-expr.md` §識別子マングリング / `docs/impl/26-codegen-effect-monad.md` §pure/return/>>= の dispatch。I7a/I7c は type class method（`show`、`==`、`>>=`、`pure`/`return`）を **runtime-shape dispatch** で回している（Ruby 側で値の tag / class を見て分岐）。properly typed dictionary passing に昇格するかは M9 以降で性能 / 型安全性の必要度を見てから。同じ枠内で spec 10-OQ1 の operator mangling 方針（`(+)` → `op_plus` vs `sapphire_op_7_6_plus`）も合わせて判断。 |
| I-OQ81 | User-defined ADT constructor value-position arity | DEFERRED-IMPL | `docs/impl/25-codegen-adt-record.md` §コンストラクタ参照。codegen が `Var { name: Upper }` を値位置で見たとき、現行は arity を AST から直接取れないので 1 引数用の curried lambda を emit（`->(x) { ADT.make(:Ctor, [x]) }`）。arity > 1 のコンストラクタを部分適用なしで値位置に書く記法は M9 例題に無いため M9 範囲では問題ない。typed AST（I-OQ57）が入れば data env から arity を引いて N 段 curry lambda を emit できる。 |
| I-OQ82 | `pure` / `return` fallback の runtime 挙動 | DEFERRED-IMPL | `docs/impl/26-codegen-effect-monad.md` §pure / return / >>= の dispatch。**specialisation の単位は enclosing top-level binding の return-type head**（例えば `main : Ruby {}` の body 全体で `pure` は `Ruby` monad に解決される）。これは `do` block 単位ではない：`main = do { xs <- listAction; pure xs }` のようにトップレベルが `Ruby` monad でも、内側 `do` が `List` に沈んでいるケースで `pure` を `List` 向けに specialise することは現行 I7c では**しない**。そうした混在ケース（M9 例題に無い）では `Sapphire::Prelude.pure_polymorphic(x)` に fallback し実行時 raise する。改善案は (a) `>>=` に先立つ monad witness 追跡（do 展開時に head を再計算）、(b) proper class dictionary。M10 以降の user コードで踏むケースが観測されたら優先度を上げる。 |
| I-OQ83 | `sapphire` CLI 引数パーサを `clap` に移すか | DEFERRED-IMPL | `docs/impl/27-cli.md` §引数パーサ。v0 は追加依存なしの手書きパーサ。`build` / `run` / `check` + `--out-dir` / `--version` / `--help` だけなので < 100 lines で足りる。`sapphire.yml`（build 04）や watch / clean / workspace flag を増やすタイミングで `clap` 採用を検討。 |
| I-OQ84 | Prelude 生成を毎 build vs 固定 snapshot | DEFERRED-IMPL | `docs/impl/24-codegen-expr.md` §prelude の取り扱い / `docs/impl/27-cli.md`。現行 `sapphire build` は `sapphire/prelude.rb` を毎回 emit する。`prelude.rb` は compiler version に一意なので、build graph に入れて "compiler が変わらなければ skip" にしてもよいが、まず incremental build の枠組みが入っていないので現状は毎回上書きで十分。I-OQ4（packaging）や incremental build 導入時に連動。 |
| I-OQ85 | 生成 Ruby の runtime version constraint 文字列を compiler build 時に固定するか | DEFERRED-IMPL | `docs/impl/24-codegen-expr.md` §File header / `docs/impl/26-codegen-effect-monad.md`。I7 reviewer follow-up (2026-04-19) で build 02 §File-content shape の契約を満たすため、生成 Ruby の header に `require_version!('~> 0.1')` 呼び出しを足した。制約文字列は現状 `codegen/mod.rs` の `RUNTIME_VERSION_CONSTRAINT` 定数に hard-code。runtime gem の minor を上げた時点でここも bumping する運用で当面足りるが、将来 packaging (D2) で compiler crate と runtime gem を同じ CI で build するようになったら `build.rs` から runtime の `lib/sapphire/runtime/version.rb` を読み込んで自動生成する方がズレが出にくい。I-OQ33（CLI と runtime gem の version 一致ポリシー）と連動して判断。 |
...
| I-OQ106 | Completion trigger character の拡張 | DEFERRED-IMPL | `docs/impl/31-lsp-completion.md` §trigger character。L6 は `trigger_characters = ["."]` のみ宣言した（英数字入力は LSP の既定で client 側が自動発火するため）。`_` や `'` は client 差が大きく、現状は追加していない。実運用で「identifier 2 文字目を打つまで popup が出ない」等の訴えが出たら追加を検討。 |
...
2026-04-19（L6 タスク、`docs/impl/31-lsp-completion.md`）で、
LSP `textDocument/completion` 導入に伴い **I-OQ106〜I-OQ109** を
§1.5 に追加。いずれも `DEFERRED-IMPL`。trigger 文字の拡張
（I-OQ106）は実運用時の訴えベース、fuzzy vs prefix ランキング
