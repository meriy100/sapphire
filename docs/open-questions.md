...
| I-OQ82 | `pure` / `return` fallback の runtime 挙動 | DEFERRED-IMPL | `docs/impl/26-codegen-effect-monad.md` §pure / return / >>= の dispatch。codegen は enclosing binding の return-type head で `pure` を specialise するが、ネストした `do` 内で違う monad の `pure` が呼ばれるケース（M9 例題に無い）では `Sapphire::Prelude.pure_polymorphic(x)` に fallback し実行時 raise する。改善案は (a) `>>=` に先立つ monad witness 追跡、(b) proper class dictionary。M10 以降の user コードで踏むケースが観測されたら優先度を上げる。 |
