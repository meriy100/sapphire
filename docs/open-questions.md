...
| I-OQ58 | `do` 展開を parse 段 / 型検査段 / codegen 段のどこでやるか | DEFERRED-IMPL | `docs/impl/20-typecheck-classes.md` §do 記法の desugar。現行は `infer_expr(Expr::Do)` 内で on-the-fly desugar。利点: AST は元の `Do` を保持するので LSP で表示可能。欠点: codegen 側も同じ desugar を再実行する必要がある。代替は "parse 直後に desugar し、`Expr::Do` を AST から消す"。I7 codegen 着手時に負担を見て判断。 |
