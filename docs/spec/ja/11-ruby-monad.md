...
### 役割呼称：作用モナド（*effect monad*）

型名 `Ruby` は散文で言語名 Ruby と衝突する（例：「Ruby モナドが
Ruby コードを走らせる」）。本文書およびチュートリアルでは、**同一
性ではなく役割を指したい場面** で本モナドを **作用モナド**（英:
*effect monad*）と呼ぶ。形式シグネチャおよび BNF では従来どおり型名 `Ruby a` を用い、
この役割呼称は会話用であり型レベルに新しい同義語を導入しない。

使い分け：

- 型位置・BNF・型付け規則：**`Ruby a`**。
- 概念を指す散文（「作用モナドのプリミティブ」「作用モナドの合成」
  など）：**作用モナド / effect monad**。
