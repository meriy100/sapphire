...
| ライセンス | 既存の `LICENSE`（MIT）を維持（I-OQ6 として dual-license を user 確認に上げる） |
...
1. `cargo check --all` — 全クレートがビルド可能か。
2. `cargo fmt --all -- --check` — フォーマット違反で fail。
...
判断を要するため **I-OQ6** として `docs/open-questions.md` §1.5
に登録し、後続で決める。I-OQ6 が `dual` 決着した場合は `LICENSE-
...
- **I-OQ6 ライセンス dual 化** — MIT 単独 vs `MIT OR Apache-2.0`。
  user 判断待ち。
